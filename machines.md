# Vulnerable Machines Catalog 2025

1000+ Free Vulnerable Machines across VulnHub, HackMyVM, VulNyx, DockerLabs, and GitHub.  
All machines are 100% free. Hack ethically. Learn continuously.
...
- [Section 1: VulnHub Machines](#section-1-vulnhub-machines)
  - [Tier 1 — Beginner (1–100)](#tier-1--beginner-machines-1100)
  - [Tier 2 — Intermediate (101–200)](#tier-2--intermediate-machines-101200)
  - [Tier 3 — Advanced (201–250)](#tier-3--advanced-machines-201250)
  - [Tier 4 — Extended Series (251–300)](#tier-4--extended-series-machines-251300)
- [Section 2: HackMyVM Machines](#section-2-hackmyvm-machines)
  - [Beginner to Intermediate (1–150)](#hackmyvm--beginner-to-intermediate-machines-1150)
  - [Intermediate to Advanced (151–200)](#hackmyvm--intermediate-to-advanced-machines-151200)
- [Section 3: VulNyx Machines](#section-3-vulnyx-machines)
- [Section 4: DockerLabs Machines](#section-4-dockerlabs-machines)
- [Section 5: GitHub Vulnerable Projects](#section-5-github-vulnerable-projects)
- [Section 6: Skill-Based Learning Paths](#section-6-skill-based-learning-paths)
- [Section 7: Certification Prep Paths](#section-7-certification-prep-paths)
- [Section 8: Strategy, Tips & Resources](#section-8-strategy-tips--resources)
- [Section 9: Progress Tracking Overview](#section-9-progress-tracking-overview)
...
|----------|------|---------------|------|-----|
| VulnHub | Downloadable VMs (OVA/VMDK) | 700+ machines | Free | vulnhub.com |
| HackMyVM | Downloadable VMs (OVA) | 300+ machines | Free | hackmyvm.eu |
| VulNyx | Downloadable VMs (OVA) | 150+ machines | Free | vulnyx.com |
| DockerLabs | Docker containers | 200+ machines | Free | dockerlabs.es |
| GitHub Projects | Docker/VM/source | 500+ apps | Free | github.com |
...
|----------|-------|---------|
| Reconnaissance | Nmap, Masscan, Rustscan, Netdiscover, arpscan | Port scanning, service detection, OS fingerprinting |
...
VulnHub provides downloadable vulnerable virtual machines. Download OVA/VMDK files and import into VirtualBox or VMware. All machines are 100% free.  
Website: [vulnhub.com](https://vulnhub.com)

### Tier 1 — Beginner (Machines #1–100)

Start here. Focus on enumeration fundamentals, basic web exploitation, simple privilege escalation.
...
| 131 | Kioptrix: 2014 VMware | FreeBSD | FreeBSD kernel privesc, Apache misconfig, exploit db | Medium |
...
| 135 | Dawn: 1 | Linux | SMB anonymous, FTP, cron script injection, privesc | Easy |
...
Advanced VulnHub machines involve exploit development, kernel exploits, multi-machine networks, or complex binary exploitation.
...
| 286 | Explore | Android | ES File Manager, ADB exploitation | Easy |
...
HackMyVM.eu provides community-created boot2root VMs. 300+ machines available, all free. Download OVA files and import into VirtualBox/VMware. No account required to download.  
...
| 41 | Joker | Linux | UDP, TFTP, Squid proxy bypass | Hard |
...
| 164 | Zerba | Linux | Web enum, priv | Easy |
| 165 | Zimba | Linux | Zimbra email, CVE exploitation | Hard |
| 166 | Zino | Linux | Web, SQL, privesc | Easy |
| 167 | Zombie | Linux | Zombie process, SUID, privesc | Easy |
| 168 | Zoom | Linux | Web, binary exploitation | Medium |
| 169 | Zulu | Linux | Multi-vector pentest | Medium |
| 170 | Zundert | Linux | Complex multi-stage | Hard |
...
| 180 | Aum | Linux | Web, binary, kernel | Hard |
| 181 | Aux | Linux | Auxiliary channels, side-channel attack | Hard |
| 182 | Ava | Linux | Web, LFI, privesc | Easy |
| 183 | Avengers | Linux | Marvel themed, multi-service, privesc | Medium |
| 184 | Avocado | Linux | Food themed, simple web, privesc | Easy |
| 185 | Axe | Linux | Web, binary exploitation | Medium |
| 186 | Aztec | Linux | Multi-stage, crypto | Hard |
| 187 | Babosco | Linux | Absolute beginner | Very Easy |
...
| 195 | Barby | Linux | Web, injection, priv | Easy |
| 196 | Baron | Linux | Web, binary RE | Medium |
| 197 | Bash | Linux | Bash scripting exploitation, eval injection | Medium |
| 198 | Bat | Linux | Web, binary | Hard |
...
VulNyx is a free cybersecurity training platform launched in 2023. Features 166 intentionally vulnerable VMs for ethical hackers. Machines are Unix-based (mostly Linux, some Windows). Download OVA format for VirtualBox.  
Website: [vulnyx.com](https://vulnyx.com)
...
DockerLabs is a free platform where all machines run as Docker containers — lightweight, fast, and beginner-friendly. No virtualization overhead. Simply download, unzip, and run.  
Website: [dockerlabs.es](https://dockerlabs.es)
...
# 3. Unzip
# 4. Run the container
bash deploy.sh <machine>.tar
# 5. Hack it, then clean up
bash destroy.sh
```
...
## Section 5: GitHub Vulnerable Projects

GitHub hosts hundreds of intentionally vulnerable applications, complete CVE recreation labs, web application security training platforms, and cloud security sandboxes. All 100% free and open source. Most run via Docker or Docker Compose.
...
| 9 | VAmPI | Python | REST API vulns, OWASP API Top 10, JWT, mass assign | Medium |
...
| Skill Focus | Recommended Machine Order | Difficulty Range |
|------------|--------------------------|-----------------|
| SQL Injection | DVWA → Sqli-labs → NullByte:1 → billu:b0x → Healthcare:1 → DC:3 → DarkHole:2 → Symfonos:5 → Prime:1 → Ouija | Easy → Hard |
| Web Application | DVWA → WebGoat → bWAPP → Mr.Robot → FristiLeaks → billu:b0x → Raven:1 → DevGuru:1 → Shuriken:1 → Corrosion:2 | Easy → Hard |
| File Inclusion (LFI/RFI) | DVWA LFI → NullByte:1 → SickOs:1.1 → The Ether → DC:5 → Symfonos:3 → Pikaboo → Interface → VulNyx LFI box | Easy → Hard |
| Privilege Escalation | Escalate_Linux:1 → Lin.Security → VulNyx (Sudo/SUID/Cron boxes) → Kioptrix series → HA:Narak → Prime:1 → Insomnia | Easy → Hard |
| Buffer Overflow | /dev/random:Scream → Brainpan:1 → Brainpan:2 → IMF:1 → VulNyx Overflow → VulNyx Format → VulNyx ROP → Fawkes → Pinky's Palace v2 | Easy → Insane |
| Active Directory | GOAD Lab → VulNyx Forest/Cascade → VulNyx Sauna/Resolute → VulNyx Blackfield → VulnNet:Roasted → BadBlood Lab | Medium → Insane |
| Container / Cloud | DockerLabs Trust → VulNyx Docker → VulNyx LXD → HackMyVM Docker → CloudGoat → KubernetesGoat → TerraGoat | Easy → Hard |
| WordPress | DC:1 → DC:2 → DC:6 → Mr.Robot → BSides Vancouver → EVM:1 → Raven:1 → HarryPotter:Aragog → ColddBox → VulNyx WordPress | Easy → Hard |
| Network Services | Kioptrix:L1 → HackLAB:Vulnix → Fowsniff → SolidState → GoldenEye:1 → Mercy → Nully Cybersecurity → Typhoon | Easy → Hard |
| OSCP Preparation | Kioptrix 1–5 → DC 1–9 → VulnOS 2 → FristiLeaks → Stapler → Lin.Security → Mr.Robot → Brainpan:1 → SolidState → Temple of Doom → Pinky's Palace v2 | Medium → Hard |
...
|----------|-----------------------|----------|-------------|-------------------|
| VulnHub | 300 machines | 100 (1–100) | 100 (101–200) | 100 (201–300) |
| HackMyVM | 200 machines | 80 machines | 80 machines | 40 machines |
| VulNyx | 166 machines | 40 machines | 50 machines | 30 machines |
| DockerLabs | 54 machines | 30 machines | 20 machines | 10 machines |
| GitHub Projects | 80 apps/labs | 30 apps | 30 apps | 20 apps/labs |
| **TOTAL** | **800+ entries** | **~280 beginner** | **~280 intermediate** | **~200 advanced** |

> VulnHub has 700+ total machines, HackMyVM has 300+, VulNyx has 166, and DockerLabs has 54+. This catalog lists 800+ curated entries from the best-known and most educational machines across all platforms. The full catalogs of each site contain thousands of additional hours of practice.
>
